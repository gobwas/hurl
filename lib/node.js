var HTTP         = require("./http"),
    ...
    TimeoutError = require("./error/timeout"),
...
exports.NodeHTTP = HTTP.extend({
    ...
    request: function(url, options) {
        ...
        commonLog = {
            href: url.href,
            uuid: this.uuid("req.out")
        };

        return new Promise(function(resolve, reject) {
            var request, response, config, proxy, start,
                data, dataLength;
            ...
            if (options.proxy) {
                ...
                if (proxy.auth) {
                    config.headers['Proxy-Authorization'] = self._getAuthString(proxy);
                }
            } else {
                config = {
    ...
    _getAuthString: function(proxy) {
        if (!proxy.auth) {
            return null;
        }

        return "Basic " + this._base64(proxy.auth.split(':').map(function(item){ return querystring.unescape(item)}).join(':'))
    },
